...
| **장애 → 복구** | 에러 업장이 임계치 이하 | 채널에 복구 알림 | PM 모달 컨펌 시 1회 |

### 핵심 규칙

- **장애 발생 문자 1회 + 해제 문자 1회** — 하나의 장애 이벤트에서 최대 2회 발송
- **모든 문자 발송은 PM 컨펌 필수** — 모달에서 문구 확인 + 확인 체크박스 2개
...
|------|--------|---------|---------|---------|
| 01075547876 | 부산 남포 더라스트호텔 | 발송완료 | 2026-02-19 08:45:29 | [안내] 현재... |

### ④ 실패 상세 (실패 건 있을 경우)
```
⚠️ 발송 실패 상세: 2건
  홍길동호텔 (010-1234-5xx): 결번 혹은 수신 거부
  제주리조트 (010-2345-6xx): 수신 거부
```
...
### 레이스 컨디션 방지
PM 두 명이 동시에 모달을 열어도 `compareAndSet`으로 1인만 통과. 이미 발송된 건이면 모달에 에러 메시지 표시.

### 테스트 모드
`.env`에 `TEST_PHONE=01012345678` 설정 시 모든 SMS가 해당 번호로만 발송. 프로덕션 전환 시 해당 줄 삭제.
